...
import { useComposerPicker } from "./composables/useComposerPicker";
...
const featureHost = useWorkspaceFeatureHostCoordinator({
  ...
  actions: {
    ...
    // 手机端外壳的 WebViewClient 会把非回环链接转成系统浏览器（见 MainActivity）。
    // 桌面端不会走到这里。
...
/**
 * 打开某本书的首选资源（短篇 / 剧本恒为「正文」目录），返回是否真的切到了那本书。
 * 新建作品后自动打开、以及输入框卡片左半边切换作品，共用这一条链路 ——
 * 短篇书籍节点本身没有 targetDocumentId，直接拿它去 selectResource 会静默什么都不做。
 */
async function selectPreferredBook(bookId: string): Promise<boolean> {
  const targetResourceId = resolvePreferredBookResourceId(
    catalogProjection.value ?? undefined,
    bookId
  );
...
// 输入框上方「书籍 / 阶段」卡片的底部面板：复用左侧栏那棵树、selectResource
// 与新建作品后自动打开的链路，不新增契约命令、不直接读写作品文件。
provide(
  COMPOSER_PICKER_CONTEXT_KEY,
  useComposerPicker({
