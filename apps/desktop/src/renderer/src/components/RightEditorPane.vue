...
import EditorCharacterCount from "./EditorCharacterCount.vue";
...
<template>
  ...
  >
    <header class="editor-header">
      ...
      <!-- 手机端：官方写作页在面包屑这一行右侧放「本书目录」胶囊；桌面三栏同屏用不着 -->
      <EditorLibraryTocButton />
    ...
    <div class="editor-toolbar">
      ...
      <EditorCharacterCount
        ...
        :recommended="recommendedContentLength"
      />
    ...
    <footer class="editor-footer">
      <div class="editor-footer-meta">
        <!--
          手机端这一份由 CSS 隐藏（字数移到工具条最右），但**必须留在 DOM 里**：
          `.editor-footer-meta > span:first-child` 的 flex 规则靠它占住首个子元素，
          抽掉的话库文档的提示 span 会顶上第一位、被 fixed 成 flex:0 0 auto 而溢出。
        -->
        <EditorCharacterCount
          :count="characterCount"
          :recommended="recommendedContentLength"
