...
/** 手机端把 header 里的「历史对话 / 新建对话」Teleport 到顶栏，见模板里的注释。 */
  ...
  <main class="conversation-pane" aria-label="智能体对话">
    <header class="conversation-header">
      ...
      <div class="conversation-header-actions">
        <!--
          手机上把这两个控件 Teleport 到顶栏的右侧槽位（MobileAppBar 里的
          #mobile-app-bar-actions），于是整个会话栏不再需要自己那条 header，
          「聊天 / 写作」顶栏只剩一条 —— 和官方手机端一致。
          桌面端 Teleport 被禁用，控件照旧留在原处，DOM 和样式都不变。
        -->
        <Teleport to="#mobile-app-bar-actions" :disabled="!mobileShell.isMobile">
          <ConversationHistoryMenu
            :conversation-history="conversationHistory"
            :current-session-id="currentSessionId"
            :responding="responding"
            :book-scoped="Boolean(agentWorkspaceType)"
            :compact="mobileShell.isMobile"
            @select-conversation="emit('selectConversation', $event)"
          />
          <button
            class="header-text-button"
            type="button"
            :aria-label="mobileShell.isMobile ? '新建对话' : undefined"
            @click="emit('newConversation')"
          >
            <AppIcon name="plus" :size="16" />
            <span v-if="!mobileShell.isMobile">新建对话</span>
          </button>
        </Teleport>
        <button
