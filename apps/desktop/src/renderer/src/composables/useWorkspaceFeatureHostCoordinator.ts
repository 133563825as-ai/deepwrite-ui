...
export function useWorkspaceFeatureHostCoordinator(
  ...
  async function openZhuqueDetection(): Promise<void> {
    if (options.isMobile()) {
      // 手机端没有 Electron 的 <webview>：把地址交给系统浏览器，由外壳的
      // WebViewClient 拦截非回环链接（见 dwdroid/.../MainActivity.java）。
      // 不切 workspaceMain —— 那个页面在手机端渲染出来只会是空白。
      options.actions.openExternalUrl(ZHUQUE_DETECTION_URL);
      return;
    }
    const generation = beginNavigation();
    if (!(await canApplyNavigation(generation))) return;
