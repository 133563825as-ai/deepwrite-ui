...
function createHarness(overrides: HarnessOverrides = {}) {
  ...
  let mobile = overrides.isMobile ?? false;
...
describe("useWorkspaceFeatureHostCoordinator", () => {
  ...
  it("朱雀检测：桌面端切页面，手机端交给系统浏览器且不切页面", async () => {
    const desktop = createHarness();
    await desktop.coordinator.openZhuqueDetection();
    expect(desktop.workspaceMainView.value).toBe("zhuque-detection");
    expect(desktop.openExternalUrl).not.toHaveBeenCalled();

    // 手机端没有 Electron 的 <webview>，切过去只会是一片空白。
    const mobile = createHarness({ isMobile: true });
    mobile.workspaceMainView.value = "conversation";
    await mobile.coordinator.openZhuqueDetection();
    expect(mobile.openExternalUrl).toHaveBeenCalledWith(ZHUQUE_DETECTION_URL);
    expect(mobile.workspaceMainView.value).toBe("conversation");
  });
